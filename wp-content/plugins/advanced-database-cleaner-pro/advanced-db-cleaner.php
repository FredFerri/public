...
class ADBC_Advanced_DB_Cleaner_Pro {
	...
	public function __construct(){
		...
		if(!defined("ADBC_PLUGIN_TYPE")) 		define("ADBC_PLUGIN_TYPE", "pro");
	...
	public function plugins_loaded(){
		...
		********************************************************************/
		...
		if(!defined("ADBC_UPLOAD_DIR_PATH_TO_ADBC")) 			define("ADBC_UPLOAD_DIR_PATH_TO_ADBC", ADBC_UPLOAD_DIR_PATH . '/adbc_uploads');
		if(!file_exists(ADBC_UPLOAD_DIR_PATH_TO_ADBC)){
			aDBc_create_folder_plus_index_file(ADBC_UPLOAD_DIR_PATH_TO_ADBC);
		}

		// Try to increase the timeout of the server to 300 to prevent multiple loads while searching
	...
	******************************************************************************************/
	...
	public static function aDBc_uninstall(){
		...
		if(ADBC_PLUGIN_TYPE == "pro"){
			...
			// Delete folder containing search results
			$aDBc_upload_dir = wp_upload_dir();
	...
	function aDBc_main_page_callback(){ ?>
		<div class="wrap">
			...
			<?php
			if(ADBC_PLUGIN_TYPE == 'pro' && !aDBc_edd_is_license_activated()){
			...
			// This is a notice for premium users who will lose pro version when upgraing to 3.0.0
			...
			<div class="<?php echo $aDBc_margin_class; ?>">
				<div class="aDBc-tab-box">
					...
					if(ADBC_PLUGIN_TYPE == "pro" || (!empty($aDBc_settings['hide_premium_tab']) && $aDBc_settings['hide_premium_tab'] == "1")){
