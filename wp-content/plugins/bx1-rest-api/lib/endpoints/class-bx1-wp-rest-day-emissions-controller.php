...
/**
 * Custom endpoint for querying for the emissions of one day.
 *
 * New filters:
 *  - `bx1_wp_rest_post_type_query` Filters the query arguments as generated
 *    from the request parameters.
 *
 */
class BX1_WP_REST_Day_Emissions_Controller extends WP_REST_Controller {

  public function __construct() {
    $this->version = '2';
    $this->namespace = 'wp/v' . $this->version;
    $this->rest_base = 'day-emissions';
  }
  ...
  public function get_items($request) {
    ...
    $args['post_type'] = array('emission'); // Le type de posts est limité dans ce flux.
   ...
   /* if (empty($args['post_type']) || !is_array($args['post_type'])) {
    ...
    if (empty($request['emissions_after'])) {
      return new WP_Error('emissions_after_need_to_be_completed', 'emissions_after is a required parameter and needs to be a ISO8601 date.', ['status' => 422]);
    }
    if (empty($request['emissions_before'])) {
      return new WP_Error('emissions_before_need_to_be_completed', 'emissions_before is a required parameter and needs to be a ISO8601 date.', ['status' => 422]);
    }
     ...
     * @var Function
     */
    $args = apply_filters("bx1_wp_rest_post_type_query", $args, $request);
    ...
    $after_mysqldate = iso8601_to_datetime($request['emissions_after']);
    $after_timestamp = mysql2date('U', $after_mysqldate);
    $before_mysqldate = iso8601_to_datetime($request['emissions_before']);
    $before_timestamp = mysql2date('U', $before_mysqldate);
    ...
    $query_args['orderby'] = array(
      'wpcf-horaire-debut' => 'asc',
      'wpcf-horaire-fin'   => 'asc',
      'modified'           => 'desc',
      'date'               => 'desc',
    );
  ...
  public function get_collection_params() {
    ...
    $params['emissions_after'] = array(
      'description' => __('Limit response to emissions published after a given ISO8601 compliant date.'),
      'type' => 'string',
      'format' => 'date-time',
      'validate_callback' => 'rest_validate_request_arg',
    );
    $params['emissions_before'] = array(
      'description' => __('Limit response to emissions published before a given ISO8601 compliant date.'),
      'type' => 'string',
      'format' => 'date-time',
      'validate_callback' => 'rest_validate_request_arg',
    );
