...
/**
 * Custom endpoint for querying for events during before a date and after another.
 *
 * New filters:
 ...
 *
 */
class BX1_WP_REST_Events_Controller extends WP_REST_Controller {
  ...
  public function get_items($request) {
    ...
    $args['post_type'] = array('evenement'); // Le type de posts est limité dans ce flux.
   ...
   /* if (empty($args['post_type']) || !is_array($args['post_type'])) {
    ...
    if (empty($request['events_after'])) {
      return new WP_Error('events_after_need_to_be_completed', 'events_after is a required parameter and needs to be a ISO8601 date.', ['status' => 422]);
    }
    if (empty($request['events_before'])) {
      return new WP_Error('events_before_need_to_be_completed', 'events_before is a required parameter and needs to be a ISO8601 date.', ['status' => 422]);
    }
    ...
    $after_mysqldate = iso8601_to_datetime($request['events_after']);
    $after_timestamp = mysql2date('U', $after_mysqldate);
    $before_mysqldate = iso8601_to_datetime($request['events_before']);
    ...
    $query_args['orderby'] = array(
      'wpcf-date-de-debut' => 'asc',
      'wpcf-date-de-fin'   => 'asc',
      'modified'           => 'desc',
      'date'               => 'desc',
    );
