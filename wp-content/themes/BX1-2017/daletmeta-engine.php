...
SetDirectory($DALETMETA, $VIDEOFILES, $WPDIR, $WPURL,$section);
function checkemission($emission_file, $emissionarray,$XML)
{
	...
	require_once($wpdirectory."/wp-admin/includes/image.php");
	require_once($wpdirectory."/wp-admin/includes/file.php");
	require_once($wpdirectory."/wp-admin/includes/media.php");
	...
	if($ifpostexist == 1)
		{
			...
			foreach ( $postexist as $page )
				...
				//Delete Picture
					...
					try {
						curl_setopt($ch, CURLOPT_URL, "https://".$wpsiteurl."/wp-content/themes/BX1-2017/daletmeta-actions.php?section=".$stage."&action=deletemedia&mediaid=" . $MediaID);
				...
				//Upload New picture
					$ch = curl_init();
					try {
						curl_setopt($ch, CURLOPT_URL, "https://".$wpsiteurl."/wp-content/themes/BX1-2017/daletmeta-upload-picture.php?action=update&postid=".$PostID."&section=".$stage."&file=".$emission_file);
						curl_setopt($ch, CURLOPT_HEADER, false);
						curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
						curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);   
						curl_setopt($ch, CURLOPT_TIMEOUT, 5);         
				...
				if ( $CurrentContent == $OriginalContent)
					{
						if (md5($Contenu) != $CurrentContent){
							...
							try {
								curl_setopt($ch, CURLOPT_URL, "https://".$wpsiteurl."/wp-content/themes/BX1-2017/daletmeta-actions.php?section=".$stage."&action=updatepost&postid=" . $PostID."&file=".$XML);
								curl_setopt($ch, CURLOPT_HEADER, false);
								curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
								curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);   
								curl_setopt($ch, CURLOPT_TIMEOUT, 5);         
	...
	else
		{
			...
			if(file_exists($videodir."/".$emission_file.".mp4"))
				{
					$video_exist = 1;
				}
			if ($code_emission != 0 && $image_exist == 1 && $video_exist == 1)
				{
					...
					$ch = curl_init();
					try {
						curl_setopt($ch, CURLOPT_URL, "https://".$wpsiteurl."/wp-content/themes/BX1-2017/daletmeta-upload-picture.php?section=".$stage."&file=".$emission_file);
						curl_setopt($ch, CURLOPT_HEADER, false);
						curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
						curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);   
						curl_setopt($ch, CURLOPT_TIMEOUT, 5);         
					...
					try {
						...
						curl_setopt($ch, CURLOPT_URL, "https://".$wpsiteurl."/wp-content/themes/BX1-2017/daletmeta-create-post.php?section=".$stage."&action=" . $action . "&codeemission=" . $code_emission . "&difdate=" . $difdate . "&attachid=". $attach_id . "&file=".$xmlfilename);
						curl_setopt($ch, CURLOPT_HEADER, false);
						curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
						curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);   
						curl_setopt($ch, CURLOPT_TIMEOUT, 5);         
