...
$VIDEOFILES = 'http://videos.bx1.be/vod';
// Temporary set default wowza url before SSL certificate is set up
$VIDEOFILES = 'https://59959724487e3.streamlock.net:443/vod';
		...
		<script>
			document.addEventListener("gestcomVideo", function(e) {
				...
				jwplayer(video_container_id).setup({
					...
					<?= get_subtitle_track(types_render_field('video_title')); ?>
					primary: 'html5',
...
function check_if_video_exists($file_id) {
	...
	$file = $VIDEOFILES . "/mp4:" . $file_id . ".mp4/playlist.m3u8";

	$httpCode = get_http_code($file);
...
function get_subtitle_track($file_id) {
	$value_show_subtitles = get_option('show_subtitles');

	if ($value_show_subtitles === "false" && !isset($_GET["showvtt"])) {
		return "";
	}

	global $VIDEOFILES;

	$file = $VIDEOFILES . "/" . $file_id . ".vtt";
