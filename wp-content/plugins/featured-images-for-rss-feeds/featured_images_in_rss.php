...
// Init Freemius.
	fifrf_fs();
	fifrf_fs()->add_filter( 'connect_message_on_update', 'firss_freemius_update_message', 10, 6 );
	fifrf_fs()->add_filter( 'connect_message', 'firss_freemius_new_message', 10, 6 );
// __End Freemius.
...
function firss_init() {
	...
	add_filter( 'post_thumbnail_html', 'firss_filter_post_thumbnail_html', 10, 5 );
...
function firss_create_parent_menu() {
	add_menu_page( 'Featured Images In RSS Feeds', 'Featured Images In RSS Feeds', 'manage_options', 'featured-images-for-rss-feeds', 'firss_settings_page', 'dashicons-images-alt' );
}
...
?>
	<div class="wrap">
		<h2></h2>
		<div class="headerDiv" >
			<a href="https://5starplugins.com/" target="_blank"><img class="headerImg" src="<?php echo plugins_url( 'includes/images/banner.jpg', __FILE__ );?>" width="940"></a>
		</div>
		...
		<form method="post" action="options.php" class="firss-settings-form">
			...
			<table class="form-table firss-settings">
				...
				<?php do_action( 'firss_settings_before' ); ?>

		        <tr valign="top">
		            <th scope="column"><?php echo  __( 'Set the size of feed images', 'featured-images-for-rss-feeds' ); ?></th>
		            <td>
			            <?php $image_sizes = apply_filters( 'firss_image_sizes', get_intermediate_image_sizes() ); ?>
						<select name="featured_images_in_rss_size">
							<?php foreach ( $image_sizes as $size_name ) : ?>
								<option value="<?php echo  esc_attr( $size_name ); ?>" <?php selected( $featured_images_in_rss_size === $size_name ); ?>><?php echo $size_name; ?></option>
							<?php endforeach; ?>
								<option value="full" <?php selected( $featured_images_in_rss_size === 'full' ); ?>>full</option>
						</select>

						<br><span class="custom-sizes" style="display: none;">
							<label for="featured_images_in_rss_thumb_size_w"><?php echo  __( 'Width', 'featured-images-for-rss-feeds' ); ?></label>
							<input name="featured_images_in_rss_thumb_size_w" type="number" step="1" min="0" id="featured_images_in_rss_thumb_size_w" value="<?php echo esc_attr( $featured_images_in_rss_thumb_size_w ); ?>" class="small-text">
							<label for="featured_images_in_rss_thumb_size_h"><?php echo  __( 'Height', 'featured-images-for-rss-feeds' ); ?></label>
							<input name="featured_images_in_rss_thumb_size_h" type="number" step="1" min="0" id="featured_images_in_rss_thumb_size_h" value="<?php echo esc_attr( $featured_images_in_rss_thumb_size_h ); ?>" class="small-text"> px
						</span>

						<p>
							<small><?php echo  sprintf( __( '(You can customize global image pixel sizes in the <a href="%1$s">Media Options</a>... then you\'ll need to <a href="%2$s" target=_blank>Regenerate Thumbnails</a>.)', 'featured-images-for-rss-feeds' ), '/wp-admin/options-media.php','http://wordpress.org/plugins/regenerate-thumbnails/' ); ?>
							</small>
						</p>
		            </td>
		        </tr>
		        <tr>
		            <th scope="column"><?php echo  __( 'Set alignment of feed images', 'featured-images-for-rss-feeds' ); ?></th>
		            <td>
		                <select name="featured_images_in_rss_css">
		                   <option value="left-above" <?php selected( $featured_images_in_rss_css === 'left-above' ); ?>><?php echo  __( 'Image Left Above Text', 'featured-images-for-rss-feeds' ); ?></option>
		                   <option value="centered-above" <?php selected( $featured_images_in_rss_css === 'centered-above' ); ?>><?php echo  __( 'Image Centered Above Text', 'featured-images-for-rss-feeds' ); ?></option>
		                   <option value="left-wrap" <?php selected( $featured_images_in_rss_css === 'left-wrap' ); ?>><?php echo  __( 'Image Left Text Wraps', 'featured-images-for-rss-feeds' ); ?></option>
		                   <option value="right-wrap" <?php selected( $featured_images_in_rss_css === 'right-wrap' ); ?>><?php echo  __( 'Image Right Text Wraps', 'featured-images-for-rss-feeds' ); ?></option>
		                </select>
		            </td>
		        </tr>
		        <tr>
		            <th scope="column"><?php echo  __( 'Set the spacing between text and feed images', 'featured-images-for-rss-feeds' ); ?></th>
		            <td>
		            	<input name="featured_images_in_rss_padding" value="<?php esc_attr_e( $featured_images_in_rss_padding ); ?>" class="small-text"> px
		            </td>
		        </tr>
				<tr>
					<th scope="column"><?php echo __( 'Enable clickable images', 'featured-images-for-rss-feeds' ); ?></th>
					<td>
						<input type="checkbox" name="featured_images_in_rss_clickable_link" <?php checked( $featured_images_in_rss_clickable_link ) ?> >
						<?php echo __( 'Adds the post URL link to the image', 'featured-images-for-rss-feeds' ) ?>
						<p><small><?php echo __( 'Outputs additional HTML surrounding the image so when clicked it links to the post URL.', 'featured-images-for-rss-feeds' ) ?></small></p>
					</td>
				</tr>

		        <?php do_action( 'firss_settings_after' ); ?>

		    </table>

		    <p class="submit"><input type="submit" name="submit-bpu" class="button-primary" value="<?php _e( 'Save Changes' ); ?>" /></p>

			<?php do_action( 'firss_settings_form_actions' ); ?>

		</form>

		<?php do_action( 'firss_settings_after_form' ); ?>

		<div class="footer-notes">
			<br/>- <?php echo sprintf( __( 'If you like this plugin, please <a href="%s" target=_blank>Rate and Review</a> it so others can benefit too.', 'featured-images-for-rss-feeds' ), 'http://wordpress.org/support/view/plugin-reviews/featured-images-for-rss-feeds/?rate=5#new-post' ); ?>
			<br/>- <?php echo sprintf( __( 'Still not seeing images in your feed? Verify featured images are set. Clear your browser and any server caches. Using Feedburner? Be sure to also <a href="%s" target=_blank>Ping Feedburner</a> so it refreshes your feed.', 'featured-images-for-rss-feeds' ),esc_url( 'http://feedburner.google.com/fb/a/pingSubmit?bloglink=' . site_url() ) ); ?>
			<br/>- <?php echo sprintf( __( 'To view your site’s raw RSS feed, click here: <a href="%s/feed/" target=_blank>/feed/</a>', 'featured-images-for-rss-feeds' ), esc_url( site_url() ) ); ?>
			<p/><?php echo sprintf( __( 'Free and premium FAQ and Support contact form available at <a href="%s" target=_blank>support.5starplugins.com</a>', 'featured-images-for-rss-feeds' ), esc_url('https://support.5starplugins.com/')); ?>
			<p/><?php echo sprintf( __( 'Developed and supported by <a href="%s" target=_blank>5 Star Plugins</a> in San Diego, CA', 'featured-images-for-rss-feeds' ), esc_url('https://5starplugins.com/')); ?> <img class="footerLogo" src="<?php echo plugins_url( 'includes/images/5StarPlugins_Logo80x80.png', __FILE__ );?>" width="20">
		</div>
	</div>
<?php
}
...
function firss_filter_post_thumbnail_html( $html, $pid, $post_thumbnail_id, $size, $attr ) {
	...
	<table class="form-table premium-features">
		<tr class="premiumHead">
			<th class="preBanner" scope="column" colspan=2>
				<h1>
					<?php echo  sprintf( __( 'Upgrade for these Premium Features%s', 'featured-images-for-rss-feeds' ), ( fifrf_fs()->is_trial() ? ' ' . __( '(Free Trial)', 'featured-images-for-rss-feeds' ) : '' ) ); ?>
				</h1>
			</th>
		</tr>

		<?php foreach ( firss_premium_features() as $feature => $desc ) : ?>
			<tr>
				<th class="preBanner" width="30%" scope="column"><span class="dashicons dashicons-yes firss-premium"></span><span class="firss-premium-feature"><?php echo $feature; ?></span></th>
					<td width="70%" scope="column"><em><?php echo $desc; ?></em>
				</th>
			</tr>
		<?php endforeach; ?>
		<tr>
			<th style="text-align: center; padding-bottom: 20px;" scope="column" colspan="2"><a class="btnBuy" href="<?php echo esc_url( firss_upgrade_url() ); ?>"><?php _e( 'Upgrade Now', 'featured-images-for-rss-feeds' ); ?></a>
			</th>
		</tr>
	</table>
<?php
...
?>
	<div class="call-to-action">
		<a class="btnBuy" href="<?php echo esc_url( firss_upgrade_url() ); ?>"><?php _e( 'Upgrade Now', 'featured-images-for-rss-feeds' ); ?></a>
		<?php if ( !fifrf_fs()->is_trial() ) : ?>
			<a class="trialLink" href="<?php echo esc_url('/wp-admin/admin.php?trial=true&page=featured-images-for-rss-feeds-pricing'); ?>"><?php echo __( '14 Day Free Trial', 'featured-images-for-rss-feeds' ); ?></a>
		<?php endif; ?>
	</div>
<?php
}
 ...
 */
function firss_freemius_update_message(	$message, $user_first_name, $plugin_title, $user_login, $site_link, $freemius_link ) {
	return sprintf(
		__fs( 'hey-x' ) . '<br>' .
		__( 'Please help us improve %2$s! If you opt-in, some data about your usage will be sent to our platform Freemius. If you skip this, that\'s okay, the %2$s will still work just fine.', 'featured-images-for-rss-feeds' ),
		$user_first_name,
		'<b>' . $plugin_title . '</b>',
		'<b>' . $user_login . '</b>',
		$site_link,
 ...
 */
function firss_freemius_new_message( $message, $user_first_name, $plugin_title, $user_login, $site_link, $freemius_link ) {
	return sprintf(
		__fs( 'hey-x' ) . '<br>' .
		__( 'In order to enjoy all of the features, functionality and enable a free trial of premium version, %2$s needs to connect your user, %3$s at %4$s, to our platform Freemius.', 'featured-images-for-rss-feeds' ),
		$user_first_name,
		'<b>' . $plugin_title . '</b>',
		'<b>' . $user_login . '</b>',
		$site_link,
