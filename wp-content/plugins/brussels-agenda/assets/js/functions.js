    ...
    if (display === 1)
    {
        if(data.media)
        {
            for(const image of data.media)
            {
                ...
                {
                    var link        = image.link;
                    var imageurl    = link.replace('https://agendabrussels.imgix.net/','/wp-content/plugins/brussels-agenda/cache/img/');
        ...
        if (imageurl === undefined)
        {
            ...
            getimage(data.categories.main.id, data.id);
        ...
        if (data.date_start === data.date_end)
            {
                datelist = data.date_start;
            }

        var bloc = `
                        <div class='calitem ` + types + `' data-city='` + data.place.translations.fr.address_zip + `' data-date='{` + datelist + `}' data-categories='{` + categories + `}' data-item-id='` + data.id + `'>
                        <div class='item-image'><a href="/wp-content/plugins/brussels-agenda/agenda-details.php?ID=` + data.id + `"><img src='` + imageurl + `'></a>
                        <span class='item-category'>` + data.categories.main.translations.fr + subcat + `</span></div>
                        <div>
...
function filterdate(date){
    ...
    jQuery('div.calitem').each(function(e){
        ...
        //console.log("Valeur de Date1 : " + dateval1);
...
function getimage(id,eventid){
    /*console.log("=== START ===");
    console.log("ID : " + id);*/
    jQuery.getJSON("/wp-content/plugins/brussels-agenda/assets/js/categories.json",function (data) {
        jQuery.each(data.category,function(key, val){
            if (val.id === id){
                picturelist = val.images
                if (picturelist !== undefined)
                {
                    randomimg = "/wp-content/plugins/brussels-agenda/img/nopics/" + picturelist[ Math.floor(Math.random() * picturelist.length) ]['file'];
                }
                else
                 {
                     randomimg = "/wp-content/plugins/brussels-agenda/img/nopicture.jpg";
                }
            }
        })
        /*console.log("ID : " + id + " // " + eventid + " || " + randomimage);
        console.log("=== END ===");*/
    });
}
function gotoURL(itemid){
    document.location.href = '/wp-content/plugins/brussels-agenda/agenda-details.php?ID=' + itemid;
    //document.location.href = '/agendaculture/detail/' + itemid;
}
jQuery(document).ready(function(){
    getimage(1);
    jQuery.getJSON("/wp-content/plugins/brussels-agenda/cache/cache.calendar.cur.json",function (data) {
